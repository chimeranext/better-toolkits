...
DEFAULT_ORG="${REPO_HYGIENE_ORG:-acme-example}"
DEFAULT_REPOS=(
...
Options:
  ...
  --repo OWNER/NAME    Limit to repo(s); repeat flag (default: ACME platform list)
...
Examples:
  ...
  ./scripts/repo-hygiene.sh prune-remote --repo acme-example/acme-docs --apply
  ./scripts/repo-hygiene.sh all --apply --local-path ~/Documentos/GitHub/client-workspace/acme-gitops
EOF
}
