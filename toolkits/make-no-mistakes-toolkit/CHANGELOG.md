...
- `/triage-security-findings` command — triage pentest-playbook + CI scanner output into owned tracker issues; pairs with `/pentest-runner`. Tenant scanner catalog in `linear-setup.json` → `securityFindingTriage` (Seacrets first consumer). No duplicate `implementation-briefs/` files.
...
- `/hygiene-hooks-setup` command + `hooks/hygiene/pre-linear-save-issue-hygiene.sh` — the hygiene-hooks layer. The hook blocks Linear `save_issue` CREATEs missing full triage (project, assignee, priority≠0, ≥1 label, milestone, explicit non-Backlog state) with exit 2; updates pass through; fail-open without `jq`. **Opt-in per repo** (house pattern): registered plugin-wide on `PreToolUse` matcher `mcp__.*linear.*__save_issue` but no-ops unless `.claude/config/hygiene-hooks.json` sets `"linear_create_hygiene": true`. The command audits a repo for the orphaned-hooks failure mode (hook scripts on disk while `settings.json` carries an empty `hooks` field — scripts never load), installs via the plugin config or a repo-level copy (merge, never clobber; lead approval for shared config), and verifies with a 7-payload synthetic suite plus the opt-in gate cases. Origin: two real incidents — issues created orphaned mid-session, and hook scripts later found orphaned on disk with an unloaded config.
- `/ready-to-review-mergeable` command — dual mode. **Issue-pipeline mode** (primary, when issue IDs are passed): drives each tracker issue to a **bot-approved, mergeable** PR via the `/implement` skill — worktree isolation + HITL on push/PR/merge, a reviewer loop gated by `Confidence ≥ threshold` (default `5.0`; `--confidence` is the relaxer, never silently raised), verdicts read from the PR's `.reviews[*]` (never `.comments[*]`), pipeline state in `.claude/.implement-prs` enforced by the `stop-prs-green.sh` Stop hook, and tracker hygiene with verify-don't-assume on MCP state writes (silent no-ops surface as a "manual flips pending" list; **never sets Done**). **Branch-ready mode** (no issue IDs): leave the current branch PR-ready and mergeable for human review — default Diátaxis front-matter `status: review` when the repo enforces that schema, run CI-equivalent validators, commit autofixes, push, open/update a non-draft PR with optional `Depends on:` chain links. **Never merges.** Complements `/implement` (execution) and `/takeover-pr` (pick up someone else's PR).
