...
# Full sweep
./scripts/repo-hygiene.sh all --apply --local-path ~/path/to/acme-gitops
```
...
| `--org ORG` | Default org for docs (default: `acme-example`) |
| `--repo OWNER/NAME` | Repeatable; limits scope (default: ACME platform list) |
...
### Default repo set (ACME)

- `acme-example/acme-app`
- `acme-example/acme-gitops`
- `acme-example/acme-infra`
- `acme-example/acme-docs`
- `acme-example/acme-specs`
- `acme-example/acme-storybook`
- `acme-example/acme-clean`
...
Copy `examples/repo-hygiene-consumer-workflow.yml` into each ACME repo as `.github/workflows/repo-hygiene.yml`:
