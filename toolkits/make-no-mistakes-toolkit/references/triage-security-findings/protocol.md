# /triage-security-findings — protocol SSOT

Harness-agnostic body. Thin entry: `commands/triage-security-findings.md`.

---

# /triage-security-findings — Triage playbook & CI security findings

You are a platform/security engineer **triaging findings** into **owned backlog items** (Linear `ACME-*` on ACME; adapt team key per tenant). This command is step **3** of the pentest trilogy — run [`/pentest-playbook-setup`](pentest-playbook-setup.md) first when config drift is unknown; pair with [`/pentest-runner`](pentest-runner.md) (execute tests).

**Playbook SSOT (ACME):** `acme-docs/how-to/operations/pentest-playbook.md`  
**Tenant config SSOT:** `acme-docs/linear-setup.json` → `securityFindingTriage`  
**Policy (team, 2026-07):** CI advisory findings (Hadolint first) → **one backlog issue per actionable finding** (dedupe by scanner + location + rule).
...
1. Read tenant `linear-setup.json` → `securityFindingTriage.scanners` (ACME: `acme-docs/linear-setup.json`).
...
## Scanner catalog (playbook ↔ ACME CI)
...
| --- | --- | --- | --- | --- | --- | --- |
| `composer-audit` | §2.1 SCA | 2 | `acme-app` | `security-dependencies.yml`, `deployment-pipeline.yml` | composer audit | mixed |
| `semgrep` | §2.2 SAST | 2 | `acme-app` | `security-static-analysis.yml` | semgrep | advisory |
| `gitleaks` | §2.3 secrets | 2 | `acme-app` | `security-scan.yml` | MegaLinter (GITLEAKS) | blocking on PR |
| `checkov` | §2.2 IaC | 2 | `acme-infra` | `security-iac.yml` | Checkov Terraform | advisory |
| `trivy-config` | §2.2 misconfig | 2 | `acme-gitops` | `security-iac.yml`, `security-scan.yml` | Trivy config | advisory |
| `trivy-fs` | §2.1 / Trivy | 2 | `acme-app` | `security-containers.yml` | Trivy filesystem | advisory |
| `trivy-image` | §2.1 container | 2 | `acme-app` | `deployment-pipeline.yml` | Trivy image SARIF | blocking path |
| `hadolint` | container hygiene | 2/7 | `acme-app` | `security-containers.yml` | Hadolint | advisory |
| `zap` | §3.1 DAST | 3 | `acme-app` | `security-dast-stage.yml` | ZAP baseline | advisory, stage |
| `dependabot` | §2.1 / OWASP-H6 | 2 | org | GitHub Dependabot API | alerts | varies |
| `phpstan-security` | §2.2 (static) | 2 | `acme-app` | `deployment-pipeline.yml` | phpstan-ci | blocking |
| `baseline-script` | §2.2 | 2 | `acme-app` | `scripts/security/run-baseline-scans.sh` | local/CI | manual invoke |
...
```bash
gh api repos/acme-example/acme-app/dependabot/alerts --jq '.[] | select(.state=="open")'
```
...
Apply labels/projects from `securityFindingTriage.scanners.{key}` in tenant config. Branch: `*/acme-{N}-*`. PR: `Fixes ACME-{N}`.
