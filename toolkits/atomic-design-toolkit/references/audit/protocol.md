...
Harness-agnostic body. Thin entry: `commands/audit.md`.
...
Note: the **vanilla** variant is very common (Laravel / Django / Rails / plain HTML projects that adopted Vite only for asset bundling). The ACME modernization pattern lives here.
...
# Vanilla (ACME pattern) — ESM modules are the unit
