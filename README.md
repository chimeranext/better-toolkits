...
For Vite **vanilla** (ACME pattern — Laravel / Django / Rails + Vite for assets) the output is a triple per component:
```
...
4. Generates component files + gallery entries + state sources, matching the detected framework:
   ...
   - **Vite vanilla** (ACME pattern) → HTML partial + ESM module (`init()`) + SCSS partial + static gallery demo
