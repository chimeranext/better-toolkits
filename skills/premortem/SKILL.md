...
description: >
  ...
  reason why. Produces a revised plan with blind spots exposed.
  MANDATORY TRIGGERS: "premortem this", "premortem my", "run a premortem",
  "what could kill this", "future-proof this", "stress test this plan",
  "what am i missing here", "find the blind spots".
  STRONG TRIGGERS: "what could go wrong", "am i missing anything",
  "poke holes in this", "where will this break", "devil's advocate this".
  Do NOT trigger on simple feedback requests, factual questions, or LLM Council
  requests. DO trigger when someone has a plan or commitment where the cost of
  being wrong is high.
---
...
The method comes from psychologist Gary Klein. He published it in [Harvard Business Review](https://hbr.org/2007/09/performing-a-project-premortem). Daniel Kahneman (the Nobel Prize-winning psychologist behind *Thinking, Fast and Slow*) called it his single most valuable decision-making technique. Google, Goldman Sachs, and Procter & Gamble all use it before major decisions.
...
- For Linear-driven workflows: any `srd/` folder, `docs/superpowers/plans/`, or referenced Linear issue IDs
...
1. `~/Escritorio/` (per `feedback_goodnight_desktop` and `feedback_standup_desktop` — the user prefers desktop for ad-hoc artifacts)
2. The current working directory if the user explicitly says so
3. NEVER a repo root unless the user explicitly says so (avoids polluting git status)
...
Open the HTML file after generating it (use `xdg-open` on Linux, `open` on macOS, `start` on Windows). If the host environment doesn't support GUI launchers, just print the absolute path.
...
- **Always spawn all failure agents in parallel.** Sequential spawning wastes time and lets earlier responses influence later ones. Issue all `Agent` tool calls in a single message.
...
- **This is not the LLM Council.** The council gives multiple perspectives on a decision *right now*. The premortem sends Claude into the future where the decision *already failed* and works backward to explain why. Different psychological mechanism, different output. If the user seems to want multiple perspectives rather than failure analysis, suggest they run the council instead.

---

## Prior art and references

- **Gary Klein** — "Performing a Project Premortem", *Harvard Business Review* (2007). Original publication of the method in management literature.
- **Daniel Kahneman** — *Thinking, Fast and Slow* (2011). Cites premortem as the single most valuable decision-making technique and explains the prospective-hindsight mechanism.
- **Mitchell & Russo (Wharton/Cornell)** — research on prospective hindsight that empirically validated the technique.
- **expectedparrot/premortem** ([github.com/expectedparrot/premortem](https://github.com/expectedparrot/premortem)) — Claude Code commands package implementing premortem flows for product launches.
- **iepathos/premortem** ([github.com/iepathos/premortem](https://github.com/iepathos/premortem)) — alternative skill implementation focused on technical decisions.
- **Microsoft prompts-for-edu** ([Team Pre-mortem Coach](https://github.com/microsoft/prompts-for-edu/blob/main/Students/Prompts/Team%20Pre-mortem%20Coach.MD)) — a coaching-style premortem prompt for student teams.

This skill borrows the explicit-frame and parallel-agent structure from the expectedparrot package, the report-and-transcript output pattern from iepathos, and the conversational context-gathering tone from the Microsoft prompt. The synthesis structure (Most Likely / Most Dangerous / Hidden Assumption / Revised Plan / Checklist) is original to this skill and is the load-bearing piece — it's what turns a list of failures into actionable revisions.
